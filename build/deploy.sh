...
mkdir lib
sshpass -p ${SSH_PASS} scp ${SSH_USER}@${SERVER_WITH_CPLEX}:cplex.jar lib/cplex.jar
...
mvn -DskipTests=true -Dcplex.jar.path=lib/cplex.jar -s ${H2_HOME}/settings.xml -Dmaven.repo.local=$M2REPOSITORY deploy